...
class GapSpider(scrapy.Spider):
    ...
    location_urls_xpath = '//ul[@id="browse-content" or @class="map-list"]/li/div/a/@href'
    store_urls_xpath = '//div[@class="map-list-wrap map-list-tall"]/ul/li//a[text()="View Store Details"]/@href'
    ...
    def parse_stores(self, response: Response) -> Generator[Dict[str, Any], None, None]:
        ...
        for store in self.get_stores(response):
            parsed_store = self._parse_store_data(response, store)
    ...
    def _parse_store_data(self, response:Response, store_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "url": response.urljoin(store_data.get("url_search_term")),
            "raw": store_data
        }
