...
class QuiktripSpider(scrapy.Spider):
    name = "quiktrip"
    allowed_domains = ["locations.quiktrip.com"]
    start_urls = ["https://locations.quiktrip.com/"]
    ...
    STORE_ID_XPATH = '//span[@id="location-name"]/span[@class="LocationName-brand"]/text()'
    ...
    HOURS_CONTAINER_XPATH = '//div[@id="hours-main-content"]//div[@class="c-hours-details-wrapper js-hours-table"]'
    ...
    HOURS_INTERVAL = './/span[@class="c-hours-details-row-intervals-instance js-hours-interval-instance"]/text()'
    SERVICES_XPATH = '//div[@class="Core-servicesListWrapper"]/ul/li/div/text()'
    ...
    def parse_store(self, response: Response) -> dict[str, Any]:
        ...
        try:
            parsed_store = {
                'number': response.xpath(self.STORE_ID_XPATH).re_first(r'#(\d+)'),
    ...
    def _get_hours(self, response: Response) -> dict[str, dict[str, str]]:
        ...
        try:
            hours = {}
            store_hours_container = response.xpath(self.HOURS_CONTAINER_XPATH)
            if not store_hours_container:
                self.logger.warning(f"Missing store hours container for: {response.url}")
                return hours

            hours_detail_rows = store_hours_container.xpath(self.HOURS_ROWS_XPATH)
            for row in hours_detail_rows:
                ...
                if not open_time or not close_time:
                    interval = self.clean_text(row.xpath(self.HOURS_INTERVAL).get()).lower()
                    if interval == "open 24 hours":
                        open_time = "12:00 am"
                        close_time = "11:59 pm"
                    else:
                        self.logger.warning(f"Missing open or close time for {full_day_name} at {response.url}")
