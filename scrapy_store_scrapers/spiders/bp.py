...
class BpSpider(scrapy.Spider):
    name = "bp"
    allowed_domains = ["map.bp.com"]
    start_urls = ["https://map.bp.com/en-GB/US/petrol-station"]
    ...
    def parse_store(self, response: Response) -> Generator[Dict[str, Any], None, None]:
        ...
        try:
            ...
            services_name_map = store_data["config"]["intlData"]["messages"]["facilities"]
            products_name_map = store_data["config"]["intlData"]["messages"]["products"]
            raw_store_info = store_data["location"]

            parsed_store = {
                "number": raw_store_info["locationId"],
                ...
                "services": self._get_services(raw_store_info, services_name_map, products_name_map),
                "url": response.url,
                "raw": store_data
            }
    ...
    def _get_address(self, store_data: dict) -> Optional[str]:
        ...
        try:
            store_info = store_data.get('location', {})
            breadcrumbs = [b_info.get('text') for b_info in store_data.get("breadcrumbs", [])]

            if len(breadcrumbs) != 5:
                self.logger.warning("Unable to extract address info from breadcrumbs: %s", store_data["location"]["locationId"])
                return None
    ...
    def _get_services(self, store_info: dict, services_name_map: dict, products_name_map:dict) -> List[str]:
        """Extract and format services."""
        try:
            services = [services_name_map[service] for service in store_info["facilities"]]
            products = [products_name_map[product] for product in store_info["products"]]

            all_services = services + products
            return all_services
    ...
    def _get_hours(self, store_info: dict) -> Dict[str, Dict[str, str]]:
        ...
        try:
            hours = {}
            hours_list = store_info.get("opening_hours", {})
