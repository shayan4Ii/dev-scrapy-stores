...
class LandSend(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for zipcode in zipcodes:
            url = f"https://www.landsend.com/pp/StoreLocator?lat={zipcode['latitude']}&lng={zipcode['longitude']}&radius=50&S=S&L=L&C=undefined&N=N"
    ...
    def parse(self, response: Response) -> Iterable[Dict]:
        if response.xpath("//markers/marker"):
            yield {
                "number": response.xpath("//markers/marker/@storenumber").get(),
                "name": response.xpath("//markers/marker/@name").get(),
                "address": self._get_address(response),
                "phone_number": response.xpath("//markers/marker/@phonenumber").get(),
                "location": {
                    "type": "Point",
                    "coordinates": [
                        float(response.xpath("//markers/marker/@lng").get()),
                        float(response.xpath("//markers/marker/@lat").get())
                    ]
                },
    ...
    def _get_hours(self, response: Response) -> List[Dict]:
        hours = {}
        days = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
        try:
            hours_string = response.xpath("//markers/marker/@storehours").get()
